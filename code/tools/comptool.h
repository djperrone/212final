...
public:
	...
	void Reset() { numLines = lineLength = totalComps = patterns_found = fPositives = 0; }
	...
	CompTool(std::string outFile,std::string code, int mod)
		: m_outFile(outFile.append(".csv")), m_inFile("na"),m_code(code), numLines(0), lineLength(0), totalComps(0), fPositives(0),m_mod(mod)
	{
		WriteHeader();
	...
	CompTool(std::string outFile, std::string code)
		: m_outFile(outFile.append(".csv")), m_inFile("na"), m_code(code), numLines(0), lineLength(0), totalComps(0), fPositives(0),m_mod(-1)
	{
		WriteHeader();
	...
	void WriteHeader() {
		...
		oStream << "file_name,num_lines,total_comps,patterns_found,false_positives,avg_line_length," << m_code << "," << m_mod << std::endl;
		oStream.close();
	}
	...
	void WriteCSV() {
		...
		oStream << m_inFile << "," << numLines << "," << totalComps << ","<<patterns_found<<"," << fPositives << "," << avgLineLength() << std::endl;
		oStream.close();
	}
