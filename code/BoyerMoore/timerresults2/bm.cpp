...
int BoyerMoore::getBadCharShift(char letter){
    ...
    //if(letter=='C')std::cout<<shift->second<<std::endl;
...
void BoyerMoore::bm(Timer& timer){
    ...
    while(align < m_string.length()){
        ...
        inner++;
        int i = key.length() - 1;
        if(align-(key.length()-1)>=9720982-10 && align-(key.length()-1)<=9720982+10){
        
            //  std::cout<<"here "<<m_string.substr(align-(key.length()-1),key.length())<<std::endl;          
       ...
       // inner = 0;
        ...
        if(align-(key.length()-1)>=9720982-15 && align-(key.length()-1)<=9720982+15){
            if (i == 0){                
            ...
            //if there isnt a match get a shift
                ...
                std::cout<<"next str "<<m_string.substr(align+1-(key.length()-1),key.length()+1)<<std::endl;          

                std::cout<<"pat "<<key<<std::endl;
               std::cout<<"char "<<m_string[align - key.length() + i + 1]<<" shift "<<getShift(i, m_string[align - key.length() + i + 1])<<std::endl;

                std::map<char, int>::iterator shift = badCharacterTable.find(m_string[align - key.length() + i + 1]);
                std::cout<<"good? "<<good<<std::endl;
                std::cout<<"bad shift "<<shift->first<<' '<<shift->second<<std::endl;
                std::cout<<"function shift "<<getShift(i, m_string[align - key.length() + i + 1])<<std::endl;
                std::cout<<getGoodSuffixShift(i)<<std::endl;
            ...
            //  std::cout<<"char "<<m_string[align - key.length() + i + 1]<<std::endl;
            }
        else{
               if (i == 0){
            align = align + key.length();
