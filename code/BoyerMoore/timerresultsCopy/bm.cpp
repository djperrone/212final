...
int BoyerMoore::getShift(int mismatchIndex, char character){
   //return max of the good/bad table shifts
    ...
    if(badShift == key.length()) badShift = key.length()-(key.length()-mismatchIndex);
...
void BoyerMoore::bm(Timer& timer){
    ...
    std::cout<<m_string.substr(2555460,30)<<std::endl;
    while(align < m_string.length()){
        ...
        // if(align-(key.length()-1)>2555476-10 && align-(key.length()-1)<2555476+10){
        //     std::cout<<"\n------------------------pre--------------------------------------\n";
                
            

        //     std::cout<<"pre align "<<align<<std::endl; 

        //     std::cout<<"str "<<m_string.substr(align-(key.length()-1),key.length())<<std::endl;
        //     std::cout<<"pat "<<key<<std::endl;
        //     std::cout<<"good shift "<<getGoodSuffixShift(i)<<std::endl;
        //     std::cout<<"bad shift "<<getBadCharShift(m_string[align - key.length() + i + 1])<<std::endl;
        //     std::cout<<"actual shift "<<getShift(i,m_string[align - key.length() + i + 1])<<std::endl;
        //     std::cout<<"post align "<<align+getShift(i,m_string[align - key.length() + i + 1])<<std::endl; 
        //     std::cout<<"\n--------------------------------------------------------------\n";
        ...
        if(align-(key.length()-1)>2555476-10 && align-(key.length()-1)<2555476+10){
            ...
            std::cout<<"str "<<m_string.substr(align-(key.length()-1),key.length())<<std::endl;
            std::cout<<"pat "<<key<<std::endl;
            std::cout<<"char "<<m_string[align - key.length() + i + 1]<<"i "<<i<<std::endl;
            std::cout<<"good shift "<<getGoodSuffixShift(i)<<std::endl;
            std::cout<<"bad shift "<<getBadCharShift(m_string[align - key.length() + i + 1])<<std::endl;
            std::cout<<"actual shift "<<getShift(i,m_string[align - key.length() + i + 1])<<std::endl;
            std::cout<<"post align "<<align+getShift(i,m_string[align - key.length() + i + 1])<<std::endl; 
       ...
       // inner = 0;
        if(!good){
            if (i == 0){
                ...
                align = align + getBadCharShift(m_string[align - key.length() + i + 1]);
            }
        }
        else{
            if (i == 0){               
                ...
                align = align + key.length();
                if(prealign == 2555477){
                    std::cout<<"wtf1 "<<align<<std::endl;
                }
            }
            else{
                if(prealign == 2555477){
                    std::cout<<"wtf2 "<<align + getShift(i, m_string[align - key.length() + i + 1])<<std::endl;
                }
                align = align + getShift(i, m_string[align - key.length() + i + 1]);
                if(prealign == 2555477){
                    std::cout<<"wtf3 "<<align<<std::endl;
                }
            }
        } 
        if(align-(key.length()-1)>2555476-10 && align-(key.length()-1)<2555476+10){
            if(i==0)std::cout<<"???\n";
            std::cout<<"actual new align "<<align<<std::endl;
        }       
