...
public:
	...
	void Reset() { numLines = lineLength = totalComps = fPositives = 0; }

	CompTool()
		:m_outFile("output.csv"), numLines(0), lineLength(0), totalComps(0), fPositives(0) {
		WriteHeader();
	}
	...
	void WriteHeader() {
		...
		oStream << "file_name,num_lines,total_comps,false_positives,avg_line_length" << std::endl;
		oStream.close();
	}
};
